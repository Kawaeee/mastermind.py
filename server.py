...
REQUIRED_PLAYERS = 2  # Minimum number of players to start the game
MAX_ROUNDS = 2  # Maximum number of rounds per player

def generate_code():
    return "".join(random.sample([str(i) for i in range(10)], 6))

def validate_guess(guess, code):
    correct_value_correct_position = sum(1 for i in range(6) if guess[i] == code[i])
    correct_value_incorrect_position = (
        sum(min(guess.count(digit), code.count(digit)) for digit in set(guess)) - correct_value_correct_position
    )
...
class GameServer:
    ...
    def handle_client(self, client_socket):
        ...
        while len(self.clients) < REQUIRED_PLAYERS:
            threading.Event().wait(1)  # Wait for a second

        # Start the game if it hasn't started yet
        if len(self.clients) == REQUIRED_PLAYERS and not self.game_started:
            self.start_game()
        ...
        while True:
            if self.game_started:
                if client_id == self.turn_index + 1:  # Adjust for 0-indexing
                    ...
                    correct_value_correct_position, correct_value_incorrect_position = validate_guess(guess, self.code)
                    ...
                    if total_rounds >= MAX_ROUNDS * len(self.clients):
    ...
    def cleanup_game(self):
        ...
        for client in self.clients[:]:  # Iterate over a copy of the list
            client.send(b"The game has ended. Disconnecting...")
            client.close()
        self.clients.clear()
